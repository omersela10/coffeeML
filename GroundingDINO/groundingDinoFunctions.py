...
class GroundingDinoApi:
    ...
    def resize_image_folder(image_path, new_image_path, size=(224, 224)):
        for filename in os.listdir(image_path):
            if filename.endswith(".jpg"):
                ...
                GroundingDinoApi.resize_image(this_image_path, output_image_path, size)
    ...
    @staticmethod
    def object_detection_get_new_image(image_path,boxes, logits, phrases,size=(224, 224)):
        try:
            with Image.open(image_path) as img:
                # Resize the image to twice the specified size
...
if __name__ == "__main__":
    model = GroundingDinoApi.load_groundingdino()
    image_path = os.path.join(os.getcwd(),'..\\Coffee_photos\\images\\1-50')
    for filename in os.listdir(image_path):
        if filename.endswith(".jpg"):
            new_image_path = os.path.join(image_path,filename)
            boxes, logits, phrases = GroundingDinoApi.object_detection(model, new_image_path)
            cropped_img = GroundingDinoApi.object_detection_get_new_image(new_image_path, boxes, logits, phrases)
            try:
